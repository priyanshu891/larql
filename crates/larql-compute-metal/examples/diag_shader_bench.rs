...
fn main() {
    ...
    let cfg = match larql_compute_metal::diag::shader_bench::Config::from_args(&args) {
        Ok(cfg) => cfg,
        Err(e) => {
            eprintln!("{e}");
            eprintln!();
            eprintln!("{}", larql_compute_metal::diag::shader_bench::usage());
            std::process::exit(2);
        }
    };

    if let Err(e) = larql_compute_metal::diag::shader_bench::run(&cfg) {
        eprintln!("{e}");
        std::process::exit(1);
    }
}
