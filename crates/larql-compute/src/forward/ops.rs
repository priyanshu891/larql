//! Forward-pass math helpers.
//!
//! The leaf helpers (`dot_proj`, `softmax`, `add_bias`) carry no arch
//! coupling. [`apply_norm`] picks RMSNorm vs LayerNorm based on
//! `arch.norm_type()` then dispatches through
//! [`crate::residual`]'s env-aware `*_for_arch` wrappers — Step 2e
//! pulled it down once `forward_overrides` followed.
...
mod tests {
    ...
    fn softmax_sums_to_one() {
        let out = softmax(&[1.0, 2.0, 3.0, 4.0]);
    ...
    #[test]
    fn softmax_largest_gets_highest_probability() {
        let out = softmax(&[0.5, 0.1, 2.0, 0.3]);
        let argmax = out
            .iter()
            .enumerate()
            ...
            .map(|(i, _)| i)
            .unwrap();
        assert_eq!(argmax, 2, "softmax argmax should match input argmax");
    }

    #[test]
    fn softmax_numerically_stable_at_large_magnitudes() {
        // Without the max-subtraction trick, exp(1000) overflows. Pin
        // that the stabilisation actually works.
        let out = softmax(&[1000.0, 1001.0, 999.0]);
    ...
    #[test]
    fn softmax_uniform_input_produces_uniform_output() {
        let out = softmax(&[2.5, 2.5, 2.5, 2.5]);
        for v in &out {
